...
- 三个参数**共享同一组解剖结构**
- 必须复用标注点，避免重复标注
- PI/PT 支持单 FH 与双 FH 两种模式，二者统一使用 `effectiveCFH` 参与计算。
...
✅ 推荐
SS :标注骶骨终板两点
PI / PT：点击后先选择“单FH”或“双FH”；已有依赖点自动继承，只提示剩余点。

## 8. 交互与同步

- PI 与 PT 共享同一组骨盆点和两个双 FH 圆，完成任一工具后自动派生另一项及 SS。
...
- 双 FH PI/PT 保存 6 点；半径控制点属于 measurement，不写入关键点层。
