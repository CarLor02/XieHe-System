...
def list_files(
    month: Optional[str] = Query(None, description="月份文件夹，如 IMG2602（已废弃）"),
    patient_folder: Optional[str] = Query(None, description="患者文件夹名称（已废弃）"),
    ...
    """
    文件列表接口（扁平化版本）
    注意：month 和 patient_folder 参数已废弃，保留仅为向后兼容
    """
    q = db.query(ScanFile)
    if is_valid is not None:
        q = q.filter(ScanFile.is_valid == is_valid)
    # 不再按 month_folder 和 patient_folder 筛选（扁平化后这两个字段为空）
    ...
    total = q.count()
    items = q.order_by(ScanFile.file_mtime.desc(), ScanFile.filename) \
