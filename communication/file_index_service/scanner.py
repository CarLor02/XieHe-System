...
扫描 WATCH_PATH 目录及所有子文件夹下的图像文件（递归）
所有文件一视同仁，最终统一转换为 PNG 导入；跳过非图像辅助文件（.sml 等）
...
def scan_once(db: Session) -> ScanResult:
    """
    扫描 WATCH_PATH 目录下的所有 DICOM 文件（单层，不递归）
    ...
    # 递归扫描目录及所有子文件夹
    for file_path in sorted(watch.rglob("*")):
        ...
        if existing is None:
            ...
            if not existing.is_primary:
                existing.is_primary = True  # 修正旧记录
                changed = True
            if changed:
