...
async def report_error(
    ...
    try:
        ...
        logger.emit_event(
            LogLevel.ERROR,
            message=f"Frontend Error Report - ID: {error_report.errorId}, "
        ...
        # 后台处理错误
        background_tasks.add_task(process_error_report, error_record)

        return success_response(
            data={"errorId": error_report.errorId}, message="错误报告已接收"
        )

    except Exception as e:
        logger.emit_event(LogLevel.ERROR, message=f"处理错误报告失败: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="处理错误报告失败"
        )
...
async def get_error_stats(
    ...
    try:
        ...
        recent_error_list = sorted(
            recent_errors, key=lambda x: x.get("serverTimestamp", ""), reverse=True
        )[:20]
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="获取错误统计失败"
        )
...
async def clear_error_logs(
    ...
    try:
        ...
        logger.emit_event(
            LogLevel.INFO,
            message=f"用户 {current_user.get('username', '未知用户')} 清空了错误日志",
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="清空错误日志失败"
        )


async def process_error_report(error_record: Dict[str, Any]) -> None:
    """
    后台处理错误报告
...
async def handle_critical_error(error_record: Dict[str, Any]) -> None:
    """处理严重错误"""
    logger.emit_event(
        LogLevel.ERROR,
        message=f"严重错误检测 - ID: {error_record.get('id')}, "
...
async def handle_network_error(error_record: Dict[str, Any]) -> None:
    """处理网络错误"""
    logger.emit_event(
        LogLevel.WARNING, message=f"网络错误检测 - {error_record.get('message')}"
    )

    # 这里可以添加网络监控逻辑


async def handle_auth_error(error_record: Dict[str, Any]) -> None:
    """处理认证错误"""
    logger.emit_event(
        LogLevel.WARNING, message=f"认证错误检测 - 用户: {error_record.get('userId')}"
    )

    # 这里可以添加安全监控逻辑


async def detect_error_patterns(error_record: Dict[str, Any]) -> None:
    """检测错误模式"""
    # 这里可以添加错误模式检测逻辑
