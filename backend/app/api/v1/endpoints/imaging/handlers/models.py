...
async def check_model_admin(
    ...
    if not (
        is_admin or "admin" in roles or "system_admin" in roles or "team_admin" in roles
    ):
        raise HTTPException(
...
async def update_configuration(
    ...
    front_model_id = request.get("front_model_id")
    if front_model_id is not None:
        update_data["front_model_id"] = front_model_id
    side_model_id = request.get("side_model_id")
    if side_model_id is not None:
        update_data["side_model_id"] = side_model_id
...
async def get_model(
    model_id: str, current_user: Dict[str, Any] = Depends(get_current_active_user)
) -> dict[str, typing.Any]:
    """获取单个模型详情"""
...
async def refresh_model_status(
    model_id: str, current_user: Dict[str, Any] = Depends(check_model_admin)
) -> dict[str, typing.Any]:
    """刷新模型状态（检查健康）"""
    try:
...
async def activate_model(
    ...
    try:
        ...
        return success_response(data=result, message="激活模型成功")
...
async def delete_model(
    ...
    try:
        ...
        return success_response(data=result, message="模型删除成功")
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
