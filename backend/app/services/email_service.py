...
class BatchEmailResult(TypedDict):
    success: int
    failed: int
    errors: list[str]
...
class EmailService:
    ...
    async def send_email(
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.INFO, message=f"邮件发送成功: {to_email} - {subject}"
            )
            return True

        except Exception as e:
            logger.emit_event(
                LogLevel.ERROR, message=f"邮件发送失败: {to_email} - {subject} - {e}"
            )
            return False
    ...
    async def send_template_email(
        ...
        if template_name not in self.templates:
            logger.emit_event(
                LogLevel.ERROR, message=f"邮件模板不存在: {template_name}"
            )
            return False
    ...
    async def send_batch_emails(
        ...
        results: BatchEmailResult = {"success": 0, "failed": 0, "errors": []}

        async def send_single_email(email_data: typing.Any) -> None:
                ...
                try:
                    ...
                    if success:
                        ...
                        results["errors"].append(
                            f"发送失败: {email_data.get('to_email', 'unknown')}"
                        )

                except Exception as e:
                    results["failed"] += 1
                    results["errors"].append(
                        f"发送异常: {email_data.get('to_email', 'unknown')} - {str(e)}"
                    )
        ...
        logger.emit_event(
            LogLevel.INFO,
            message=f"批量邮件发送完成: 成功 {results['success']}, 失败 {results['failed']}",
        )
        return results
    ...
    async def _add_attachment(self, message: MIMEMultipart, file_path: str) -> None:
        ...
        try:
            ...
            if not attachment_path.exists():
                logger.emit_event(
                    LogLevel.WARNING, message=f"附件文件不存在: {attachment_path}"
                )
                return
            ...
            logger.emit_event(
                LogLevel.ERROR, message=f"添加附件失败: {file_path} - {e}"
            )
...
async def send_welcome_email(username: str, email: str, login_url: str) -> bool:
    ...
    return await email_service.send_template_email(
        ...
        context={"username": username, "email": email, "login_url": login_url},
        to_name=username,
    )


async def send_password_reset_email(
    username: str, email: str, reset_url: str, expires_in: int = 24
) -> bool:
    """发送密码重置邮件"""
    return await email_service.send_template_email(
        to_email=email,
