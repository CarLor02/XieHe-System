...
class MetricsCollector:
    ...
    def __init__(self, max_size: int = 1000) -> None:
        ...
        self.metrics: deque[PerformanceMetric] = deque(maxlen=max_size)
        self.api_response_times: deque[PerformanceMetric] = deque(maxlen=max_size)
        self.db_query_times: deque[PerformanceMetric] = deque(maxlen=max_size)
        self.system_metrics: deque[PerformanceMetric] = deque(maxlen=max_size)
    ...
    def get_recent_metrics(
        self, metric_type: str | None = None, minutes: int = 60
...
class MonitoringService:
    ...
    async def record_api_response_time(
        ...
        if response_time > self.thresholds["api_response_time"]:
            logger.emit_event(
                LogLevel.WARNING,
                message=f"API响应时间过长: {endpoint} {method} - {response_time:.2f}s",
            )

    async def record_db_query_time(
        self, query_type: str, table_name: str, query_time: float
    ) -> None:
        """记录数据库查询时间"""
        metric = PerformanceMetric(
            timestamp=datetime.now(),
            ...
            value=query_time,
            unit="seconds",
            tags={"query_type": query_type, "table": table_name},
        )
        self.collector.add_metric(metric)
        ...
        if query_time > self.thresholds["db_query_time"]:
            logger.emit_event(
                LogLevel.WARNING,
                message=f"数据库查询时间过长: {table_name} {query_type} - {query_time:.2f}s",
            )
    ...
    def get_metrics_history(
        self, metric_type: str | None = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """获取指标历史数据"""
...
async def _record_async_performance(
    endpoint: str | None,
    query_type: str | None,
    elapsed: float,
    status_code: int = 200,
    ...
    elif query_type and status_code < 400:
...
def _async_performance_wrapper(
    func: typing.Any,
    endpoint: str | None,
    query_type: str | None,
) -> typing.Any:
    async def wrapper(*args: typing.Any, **kwargs: typing.Any) -> typing.Any:
        ...
        try:
            ...
            await _record_async_performance(
                endpoint,
                query_type,
            ...
            raise
        await _record_async_performance(endpoint, query_type, time.time() - started_at)
        return result

    return wrapper


def _sync_performance_wrapper(func: typing.Any, query_type: str | None) -> typing.Any:
    def wrapper(*args: typing.Any, **kwargs: typing.Any) -> typing.Any:
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.ERROR,
                message=f"监控的函数执行失败: {func.__name__} - {elapsed:.2f}s - {exc}",
            )
            raise
        if query_type:
            asyncio.create_task(
                monitoring_service.record_db_query_time(
                    query_type=query_type,
                    table_name="unknown",
                    query_time=time.time() - started_at,
                )
            )
        return result
...
def monitor_performance(
    endpoint: str | None = None, query_type: str | None = None
) -> typing.Any:
    """性能监控装饰器"""

    def decorator(func: typing.Any) -> typing.Any:
        if asyncio.iscoroutinefunction(func):
            return _async_performance_wrapper(func, endpoint, query_type)
        return _sync_performance_wrapper(func, query_type)
