...
class AlertRule:
    ...
    def trigger(self, data: Dict[str, Any]) -> Alert:
        ...
        self.last_triggered = triggered_at

        # 生成告警ID
        alert_id = f"{self.name}_{int(triggered_at.timestamp())}"
...
class AlertService:
    ...
    def __init__(self) -> None:
        ...
        self.check_task: asyncio.Task[None] | None = None
    ...
    async def _handle_alert(self, alert: Alert) -> None:
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.WARNING, message=f"触发告警: {alert.title} - {alert.message}"
            )
        ...
        try:
            ...
            for email in subscribers:
                try:
                    ...
                    logger.emit_event(
                        LogLevel.ERROR, message=f"发送告警邮件失败: {email} - {e}"
                    )
    ...
    async def _cleanup_old_alerts(self) -> None:
        ...
        try:
            ...
            if expired_alerts:
                logger.emit_event(
                    LogLevel.INFO, message=f"清理了 {len(expired_alerts)} 个过期告警"
                )
    ...
    def subscribe(self, email: str, levels: List[AlertLevel]) -> None:
        """订阅告警通知"""
        for level in levels:
        ...
        logger.emit_event(
            LogLevel.INFO,
            message=f"用户 {email} 订阅了告警级别: {[level.value for level in levels]}",
        )

    def unsubscribe(self, email: str, levels: List[AlertLevel] | None = None) -> None:
        """取消订阅告警通知"""
        if levels is None:
            # 取消所有级别的订阅
    ...
    def suppress_alert(self, alert_id: str, minutes: int = 60) -> bool:
        ...
        if alert_id in self.alerts:
            ...
            self.alerts[alert_id].suppressed_until = datetime.now() + timedelta(
                minutes=minutes
            )
            logger.emit_event(
                LogLevel.INFO, message=f"告警已抑制 {minutes} 分钟: {alert_id}"
            )
            return True
        return False
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # 按级别统计
        level_stats: defaultdict[str, int] = defaultdict(int)
        for alert in self.alerts.values():
            level_stats[alert.level.value] += 1

        # 按状态统计
        status_stats: defaultdict[str, int] = defaultdict(int)
        for alert in self.alerts.values():
