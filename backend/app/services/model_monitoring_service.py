...
class ModelMonitoringService:
    ...
    def __init__(self) -> None:
        self.metrics: Dict[str, deque[ModelMetric]] = defaultdict(
            lambda: deque(maxlen=1000)
        )
        self.errors: Dict[str, deque[ModelError]] = defaultdict(
    ...
    async def record_inference_time(
        ...
        metric = ModelMetric(
            ...
            metadata={"input_size": input_size, "output_size": output_size},
        )
        ...
        if inference_time > self.thresholds["inference_time"]:
            logger.emit_event(
                LogLevel.WARNING,
                message=f"模型 {model_name} 推理时间过长: {inference_time:.2f}s",
            )
    ...
    async def record_model_error(
        ...
        logger.emit_event(
            LogLevel.ERROR,
            message=f"模型 {model_name} 发生错误: {error_type} - {error_message}",
        )

    async def record_throughput(
        self, model_id: str, model_name: str, requests_per_second: float
    ) -> None:
        """记录模型吞吐量"""
        metric = ModelMetric(
            model_id=model_id,
    ...
    async def _update_model_status(self, model_id: str) -> None:
        """更新模型状态"""
        try:
            ...
            # 计算统计信息
            inference_times = [
                m.value for m in recent_metrics if m.metric_type == "inference_time"
            ]
            accuracies = [
                m.value for m in recent_metrics if m.metric_type == "accuracy"
            ]
            throughputs = [
                m.value for m in recent_metrics if m.metric_type == "throughput"
            ]
            ...
            error_rate = (
                (len(recent_errors) / total_requests * 100) if total_requests > 0 else 0
            )
            ...
            if inference_times:
                ...
                status["p95_inference_time"] = (
                    statistics.quantiles(inference_times, n=20)[18]
                    if len(inference_times) >= 20
                    else max(inference_times)
                )
            ...
            logger.emit_event(
                LogLevel.ERROR, message=f"更新模型 {model_id} 状态失败: {e}"
            )

    async def _check_model_alerts(self) -> None:
        """检查模型告警"""
        for model_id, status in self.model_status.items():
            ...
            for alert in alerts:
                logger.emit_event(
                    LogLevel.WARNING, message=f"模型 {model_id} 告警: {alert}"
                )
    ...
    def get_model_metrics(
        self, model_id: str, metric_type: str | None = None, hours: int = 24
    ) -> List[Dict[str, Any]]:
        """获取模型指标历史"""
    ...
    def get_model_errors(self, model_id: str, hours: int = 24) -> List[Dict[str, Any]]:
        """获取模型错误历史"""
    ...
    def get_model_statistics(self, model_id: str) -> Dict[str, Any]:
        ...
        # 按类型分组指标
        metrics_by_type: defaultdict[str, list[float]] = defaultdict(list)
    ...
    def update_thresholds(self, new_thresholds: Dict[str, float]) -> None:
        ...
        logger.emit_event(
            LogLevel.INFO, message=f"模型监控阈值已更新: {new_thresholds}"
        )
...
# 便捷函数
async def monitor_model_inference(model_id: str, model_name: str) -> typing.Any:
    """模型推理监控装饰器"""
