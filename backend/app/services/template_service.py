...
class TemplateService:
    ...
    def _process_conditions(template: str, data: Dict[str, Any]) -> str:
        ...
        def replace_condition(match: typing.Any) -> typing.Any:
    ...
    def _evaluate_condition(condition: str, data: Dict[str, Any]) -> bool:
        ...
        for op in operators:
            ...
            if len(parts) != 2:
                continue
            left_value = TemplateService._get_value(parts[0].strip(), data)
            right_value = parts[1].strip().strip("\"'")
            return TemplateService._compare_values(left_value, right_value, op)
    ...
    def _replace_variables(template: str, data: Dict[str, Any]) -> str:
        ...
        def replace_var(match: typing.Any) -> str:
    ...
    def _extract_measurement_data(
        measurements: List[Any], exam_type: str
    ) -> Dict[str, Any]:
        """
        从测量数据中提取报告所需的数据
        ...
        if exam_type == "正位X光片":
            ...
            data["Nash_Moe_Grade"] = measurement_dict.get("Nash-Moe", {}).get(
                "value", 0
            )
            data["T1_Tilt"] = measurement_dict.get("T1 Tilt", {}).get("value", 0.0)
            data["CA_Value"] = measurement_dict.get("CA", {}).get("value", 0.0)
            data["Pelvic_Obliquity"] = measurement_dict.get("Pelvic", {}).get(
                "value", 0.0
            )
            ...
            if cobb_angle >= 10:
                data["Summary_Text"] = (
                    f"{data['Severity_Level']}，Cobb角{cobb_angle:.1f}°"
                )
                if trunk_shift > 10:
