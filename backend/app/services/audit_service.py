...
class AuditService:
    ...
    def __init__(self) -> None:
        self.log_queue: asyncio.Queue[AuditLog] = asyncio.Queue()
        self.batch_size = 100
    ...
    async def _flush_batch(self, batch: List[AuditLog]) -> None:
        ...
        try:
            ...
            db.commit()
            logger.emit_event(
                LogLevel.INFO, message=f"成功写入 {len(batch)} 条审计日志"
            )
    ...
    async def get_statistics(
        ...
        try:
            ...
            total_result = db.execute(
                text(total_sql), {"start_time": start_time, "end_time": end_time}
            ).fetchone()
            ...
            action_result = db.execute(
                text(action_sql), {"start_time": start_time, "end_time": end_time}
            ).fetchall()
            ...
            resource_result = db.execute(
                text(resource_sql), {"start_time": start_time, "end_time": end_time}
            ).fetchall()
            ...
            level_result = db.execute(
                text(level_sql), {"start_time": start_time, "end_time": end_time}
            ).fetchall()
            ...
            user_result = db.execute(
                text(user_sql), {"start_time": start_time, "end_time": end_time}
            ).fetchall()
