...
class DICOMProcessor:
    ...
    def __init__(self) -> None:
        if not PYDICOM_AVAILABLE:
            raise ImportError(
                "pydicom库未安装，请在 backend 目录运行: uv sync --frozen"
            )
    ...
    def validate_dicom_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.WARNING, message=f"DICOM文件验证失败 {file_path}: {e}"
            )
            return False
    ...
    def extract_metadata(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            metadata = {
                ...
                # 文件信息
                "transfer_syntax_uid": getattr(ds, "file_meta", {}).get(
                    "TransferSyntaxUID", ""
                ),
                "implementation_class_uid": getattr(ds, "file_meta", {}).get(
            ...
            logger.emit_event(
                LogLevel.INFO, message=f"DICOM元数据提取成功: {file_path}"
            )
            return metadata

        except Exception as e:
            logger.emit_event(
                LogLevel.ERROR, message=f"DICOM元数据提取失败 {file_path}: {e}"
            )
            return {}

    def extract_pixel_data(self, file_path: Path) -> Optional[NDArray[np.generic]]:
        """
        提取DICOM像素数据
        ...
        try:
            ...
            if not hasattr(ds, "pixel_array"):
                logger.emit_event(
                    LogLevel.WARNING, message=f"DICOM文件无像素数据: {file_path}"
                )
                return None
            ...
            if hasattr(ds, "WindowCenter") and hasattr(ds, "WindowWidth"):
                ...
                pixel_array = (
                    (pixel_array - window_min) / (window_max - window_min) * 255
                ).astype(np.uint8)

            logger.emit_event(
                LogLevel.INFO,
                message=f"DICOM像素数据提取成功: {file_path}, shape: {pixel_array.shape}",
            )
            return pixel_array

        except Exception as e:
            logger.emit_event(
                LogLevel.ERROR, message=f"DICOM像素数据提取失败 {file_path}: {e}"
            )
            return None

    def convert_to_image(
        self, file_path: Path, output_format: str = "PNG"
    ) -> Optional[bytes]:
        """
        将DICOM转换为标准图像格式
        ...
        """
        try:
            extracted_pixels = self.extract_pixel_data(file_path)
            if extracted_pixels is None:
                return None
            pixel_array: NDArray[np.generic] = extracted_pixels
            ...
            logger.emit_event(
                LogLevel.INFO, message=f"DICOM转换为{output_format}成功: {file_path}"
            )
    ...
    def generate_thumbnail(
        self, file_path: Path, size: Tuple[int, int] = (256, 256)
    ) -> Optional[bytes]:
        """
        生成DICOM缩略图
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.INFO, message=f"DICOM缩略图生成成功: {file_path}, size: {size}"
            )
            return output_buffer.getvalue()

        except Exception as e:
            logger.emit_event(
                LogLevel.ERROR, message=f"DICOM缩略图生成失败 {file_path}: {e}"
            )
            return None
        ...
        try:
            ...
            if hasattr(ds, "pixel_array"):
                try:
                    ...
                    logger.emit_event(
                        LogLevel.WARNING, message=f"获取像素数组信息失败: {e}"
                    )
            ...
            logger.emit_event(
                LogLevel.ERROR, message=f"获取DICOM信息失败 {file_path}: {e}"
            )
            return {"error": str(e)}

    def batch_process(
        self, file_paths: List[Path], extract_thumbnails: bool = True
    ) -> List[Dict[str, Any]]:
        """
        批量处理DICOM文件
        ...
        for file_path in file_paths:
            try:
                ...
                logger.emit_event(
                    LogLevel.ERROR, message=f"批量处理DICOM文件失败 {file_path}: {e}"
                )
                results.append(
                    {
                        "file_path": str(file_path),
