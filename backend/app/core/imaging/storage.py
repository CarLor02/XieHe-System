...
class StorageManager:
    ...
    def generate_file_path(
        self, filename: str, category: str = "general", patient_id: Optional[str] = None
    ) -> str:
        """生成文件存储路径"""
        # 按日期分目录
    ...
    def file_exists(self, file_path: str, backend: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.ERROR, message=f"检查文件存在异常 {file_path}: {e}"
            )
            return False
    ...
    def get_file_url(
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.ERROR, message=f"获取文件URL异常 {file_path}: {e}"
            )
            return ""

    def list_files(
        self, prefix: str = "", limit: int = 100, backend: Optional[str] = None
    ) -> List[dict[str, typing.Any]]:
        """列出文件"""
        try:
            storage_backend = self.get_backend(backend)
    ...
    def cleanup_temp_files(self, older_than_hours: int = 24) -> int:
        ...
        try:
            ...
            logger.emit_event(
                LogLevel.INFO, message=f"清理临时文件完成: {cleaned_count} 个文件"
            )
