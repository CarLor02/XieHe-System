...
def check_users():
    ...
    with engine.connect() as conn:
        ...
        if not users:
            ...
            # 表头
            print(
                f"{'ID':<5} {'用户名':<15} {'邮箱':<30} {'姓名':<15} {'状态':<10} {'角色':<10} {'创建时间':<20}"
            )
            print("-" * 120)
            ...
            for user in users:
                ...
                print(
                    f"{user_id:<5} {username:<15} {email:<30} {real_name:<15} {status_display:<10} {role:<10} {str(created_at):<20}"
                )
        ...
        result = conn.execute(
            text(
                "SELECT COUNT(*) FROM users WHERE status = 'active' AND (is_deleted = 0 OR is_deleted IS NULL)"
            )
        )
        ...
        result = conn.execute(
            text(
                "SELECT COUNT(*) FROM users WHERE is_superuser = 1 AND (is_deleted = 0 OR is_deleted IS NULL)"
            )
        )
        ...
        result = conn.execute(
            text(
                "SELECT COUNT(*) FROM users WHERE is_verified = 1 AND (is_deleted = 0 OR is_deleted IS NULL)"
            )
        )
