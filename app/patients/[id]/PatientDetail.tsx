...
export default function PatientDetail({ patientId }: { patientId: string }) {
  ...
  const updateDiagnosis = (type: keyof typeof diagnoses, field: 'status' | 'value', value: any) => {
    setDiagnoses(prev => ({
      ...prev,
      [type]: {
        ...prev[type],
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-gray-900 font-medium">{mockPatientDetail.name}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">患者ID</label>
                    <p className="text-gray-900">{mockPatientDetail.patientId}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{mockPatientDetail.birthDate}</p>
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-gray-900">{mockPatientDetail.emergencyContact}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-500 mb-1">紧急联系电话</label>
                    <p className="text-gray-900">{mockPatientDetail.emergencyPhone}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                <h3 className="text-lg font-semibold text-gray-900 mb-4">就诊统计</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">总检查次数</span>
                    <span className="text-2xl font-bold text-blue-600">{mockPatientDetail.totalExams}</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-gray-600">最近就诊</span>
                    <span className="text-gray-900">{mockPatientDetail.lastVisit}</span>
                  </div>
                  <div className="flex items-center justify-between">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-500 mb-2">既往病史</label>
              ...
              {Object.entries(diagnoses).map(([type, diagnosis]) => {
                ...
                return (
                  <div key={type} className="border border-gray-200 rounded-lg p-4 hover:shadow-sm transition-shadow">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        {isEditingDiagnoses ? (
                          <input
                            ...
                            onChange={(e) => updateDiagnosis(type as keyof typeof diagnoses, 'status', e.target.checked)}
                            className="rounded text-blue-600 focus:ring-blue-500"
                          />
                        ) : (
                          <span className={`text-xs px-2 py-1 rounded-full ${getDiagnosisStatusColor(type, diagnosis.status)}`}>
                    ...
                    <div className="mb-3">
                      <h4 className="text-sm font-medium text-gray-700 mb-1">{info?.fullName}</h4>
                      <p className="text-xs text-gray-600">{info?.description}</p>
                    </div>
                    
                    <div className="mb-3">
                      <label className="block text-xs font-medium text-gray-500 mb-1">检查结果</label>
                      {isEditingDiagnoses ? (
                        <input
                          type="text"
                          value={diagnosis.value}
                          onChange={(e) => updateDiagnosis(type as keyof typeof diagnoses, 'value', e.target.value)}
                    ...
                    <div className="text-xs text-gray-500 bg-blue-50 p-2 rounded">
                      <span className="font-medium">参考范围:</span> {info?.normalRange}
                    </div>
                  </div>
                );
              })}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-900">检查日期</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-900">检查类型</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-900">主治医生</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-900">诊断结果</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {mockExamRecords.map((record) => (
                    <tr key={record.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4 text-sm text-gray-900">{record.doctor}</td>
                      <td className="px-4 py-4 text-sm text-gray-900">{record.diagnosis}</td>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-96">
              ...
              <p className="text-gray-600 mb-6">
                确定要删除患者 "{mockPatientDetail.name}" 的所有信息吗？此操作不可恢复。
              </p>
