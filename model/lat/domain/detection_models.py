...
class VertebraDetection(BaseModel):
    ...
    bbox: list[float] = Field(
        ..., description="边界框 [x_center, y_center, width, height]，归一化坐标"
    )
    keypoints: list[Point] = Field(
        ..., description="椎体4个角点；S1为上终板左右2点（归一化）"
    )
...
class CFHDetection(BaseModel):
    ...
    s1_left: Point = Field(..., description="S1上终板图像左侧点（归一化）")
    s1_right: Point = Field(..., description="S1上终板图像右侧点（归一化）")
...
class DetectionResponse(BaseModel):
    ...
    cfh: CFHDetection | None = Field(None, description="股骨头检测结果")
