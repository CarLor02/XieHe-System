...
function collectTextContents(node: ReactNode): string[] {
  ...
  if (isValidElement<TextProps>(node)) {
    const ownText =
      node.type === 'text' ? [textContent(node.props.children)] : [];
...
function transformContext(containerSize: {
  width: number;
  height: number;
}): TransformContext {
  return {
...
it('keeps SVA guide line endpoints fixed in image coordinates across viewport sizes', () => {
  ...
  const desktopGuide = renderMeasurementLines(
    measurement,
    desktopContainer
  ).find(
    line =>
  ...
  const compactGuide = renderMeasurementLines(
    measurement,
    compactContainer
  ).find(
    line =>
  ...
  const desktopEnd = lineEndToImagePoint(
    desktopGuide!,
    desktopContainer,
    'end'
  );
  const compactEnd = lineEndToImagePoint(
    compactGuide!,
    compactContainer,
    'end'
  );
...
it('keeps LLD horizontal guide endpoints fixed in image coordinates across viewport sizes', () => {
  ...
  const desktopStart = lineEndToImagePoint(
    desktopGuide!,
    desktopContainer,
    'start'
  );
  const compactStart = lineEndToImagePoint(
    compactGuide!,
    compactContainer,
    'start'
  );
