...
it('replaces Cobb endpoint keypoints with the Cobb measurement points', () => {
  ...
  const synced = syncCobbMeasurementToKeypoints(existingKeypoints, measurement);

  expect(
    synced
      ?.filter(keypoint =>
        ['T2-1', 'T2-2', 'T10-3', 'T10-4'].includes(keypoint.id)
      )
      .map(keypoint => ({
        id: keypoint.id,
...
it('uses anterior endpoint keypoint order for anterior Cobb even when endpoints match a lateral named Cobb', () => {
  ...
  const synced = syncCobbMeasurementToKeypoints([], measurement, '正位X光片');
