...
export const TPA_CONFIG: AnnotationConfig = {
  ...
  calculateResults: (points: Point[], context: CalculationContext) => {
    ...
    // 计算从点2到中点的向量
    const v2 = {
      x: midPoint.x - points[1].x,
      y: midPoint.y - points[1].y
    };
  ...
  getLabelPosition: (points: Point[], imageScale: number = 1) => {
    ...
    return {
      x: (points[0].x + points[1].x + midX) / 3,
      y: (points[0].y + points[1].y + midY) / 3 - 20 / imageScale
    };
  },
  ...
  isInHoverRange: (mousePoint: Point, points: Point[], tolerance: number = 10) => {
    ...
    return isPointNearLine(mousePoint, points[1], points[0], tolerance) ||
           isPointNearLine(mousePoint, points[1], midPoint, tolerance);
  },
